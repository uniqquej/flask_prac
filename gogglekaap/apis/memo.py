...
post_parser.add_argument('labels', action='split', help = '라벨 번호 콤마 스트링') #리스트로 된 라벨스를 라벨로 잘라주는 역할 (split)
...
get_parser.add_argument('label', help='라벨번호')
    ...
    def get(self):
        ...
        if label:
            base_query = base_query.filter(
                Memo.labels.any(LabelModel.id == label)
            )
